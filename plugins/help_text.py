...
async def start_me(bot, update):
    ...
    if update_channel:
        try:
            user = await bot.get_chat_member(update_channel, update.chat.id)
            if user.status == "kicked":
               await update.reply_text(" Sorry,You've Been Flooding Me So My Owner Removed You From Using Me If You Think It's An Error Contact : @cant_think_1")
               return
        except UserNotParticipant:
            await update.reply_text(
                text="**Due To The Huge Traffic Only Channel Members Can Use This Bot Means You Need To Join The Below Mentioned Channel Before Using Me! **",
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                ...
                [
                    InlineKeyboardButton('Support Channel', url='https://t.me/Wizard_Bots'),
                    InlineKeyboardButton('Feedback', url='https://t.me/WizardBotHelper')
                ],
            ]
        ),
